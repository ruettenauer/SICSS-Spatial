...
<!-- And we add income estimates (https://data.london.gov.uk/dataset/ons-model-based-income-estimates--msoa) -->

<!-- ```{r} -->

<!-- # Download excel file (why...??) -->
<!-- econ.link <- "http://data.london.gov.uk/download/ons-model-based-income-estimates--msoa/ad11a43a-048b-437c-9f7e-116df59eb811/ons-model-based-income-estimates-msoa.xls" -->
<!-- econ.df <- gdata::read.xls(econ.link, sheet = 2) # this only works with http, not with https -->
<!-- names(econ.df) <- gsub("\\.\\.+", "", names(econ.df)) -->
<!-- econ.df[, 7:ncol(econ.df)] <- sapply(econ.df[, 7:ncol(econ.df)],  -->
<!--                                      FUN = function(x) as.numeric(gsub(",", "", x))) -->

<!-- # Merge (as with conventional df) -->
<!-- msoa.spdf <- merge(msoa.spdf, econ.df,  -->
<!--                    by.x = "MSOA11CD", by.y = "MSOA.code",  -->
<!--                    all.x = TRUE, all.y = FALSE) -->

<!-- mapview(msoa.spdf[, "Total.annual.income"])   -->
<!-- ``` -->

And we add the median housing prices in 2017 from the [London Datastore](https://data.london.gov.uk/dataset/average-house-prices)
...
We might also be interested in the average tree cover density within 1 km radius around each MSOA centroid. Therefore, we first create a buffer with `st_buffer()` around each midpoint and subsequently use `st_inter()`.
...
Note: For buffer related methods it often makes sense to use a population weighted centroid instead of geographic centroid (see [here](https://geoportal.statistics.gov.uk/datasets/ons::middle-layer-super-output-areas-december-2011-population-weighted-centroids/about) for MSOA population weighted centroids). However, often this information is not available.

# Interpolation and Kriging

For (sparse) point data, we often have the problem that the nearest count point might be far away from where we want to measure or merge its attributes. One way of dealing with this problem is to spatially interpolate the data on regular grid given: given a specific function of space (and covariates), we make prediction about an attribute at "missing" locations. For more details, see for instance [Spatial Data Science](https://keen-swartz-3146c4.netlify.app/interpolation.html) or [Introduction to Spatial Data Programming with R](https://keen-swartz-3146c4.netlify.app/interpolation.html).
...
There are various ways of interpolating. Common methods are nearest neighbours matching or inverse distance weighted interpolation (e.g. using `idw()`), where the value at a given point is a weighted average of surrounding values, with weights being a function of distance.
...
A more advantage technique is Kriging, which estimates values as a function of a deterministic trend a random process. However, we have to set some hyper-parameters for that: sill, range, nugget, and the functional form. Therefore, we first need to fit a semi-variogram. Subsequently, we let the `fit.variogram()` function chose the the parameters based on the empirical variogram [see for instance [r-spatial Homepage](https://r-spatial.org/r/2016/02/14/gstat-variogram-fitting.html)].
...
The example above is a little bit tricky, given that traffic does not follow a random process, but (usually) follows the street network. We would probably increase the performance by either adding the street network itself and interpolating along this network, or by adding covariates to our prediction (Universal Kriging). Alternatively, we can use integrated nested Laplace approximation with [R-INLA](https://becarioprecario.bitbucket.io/inla-gitbook/index.html) [@GomezRubio.2020].

Lets add the prediction to our original msoa data using the insection operation as above.
