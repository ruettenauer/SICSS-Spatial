...
Instead of using exemplary data shipped with the packages, the course relies on real world data sources from the [London Datastore](https://data.london.gov.uk/dataset).

### Why is spatial data linkage and analysis part of SICSS? 

The amount of spatial ('readymade') data sources is steadily increasing. For instance, the UK open data portal provides many indactors on a spatially agrregated level: https://data.gov.uk/. Moreover, geographical information is increasingly available for traditional ('custommade') data sources, such as survey data. Using geographic information allows us to:
